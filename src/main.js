...
import steemconnect from "sc2";
import UIkit from "vuikit";
// import App from "./App";
import replaceMedia from "./helpers/replaceMedia";
import calculateReputation from "./helpers/calculateReputation";
...
const postMixin = {
    ...
    computed: {
        ...
        payout: function () {
            if (this.post.last_payout === "1970-01-01T00:00:00") {
                const payout = this.post.pending_payout_value.replace(" SBD", "");
                return parseFloat(payout).toFixed(2);
            }

            const authorPayout = this.post.total_payout_value.replace(" SBD", "");
            const curatorPayout = this.post.curator_payout_value.replace(" SBD", "");
    ...
    methods: {
        ...
        submitReply: function (e) {
            ...
            if (this.account) {
                this.replying = true;
                const replyTextarea = $(e.target).find(".reply-textarea");
                const reply = replyTextarea.val();
                const date = new Date();
                const replyPermlink = "re-" + this.post.author + "-" + this.post.permlink + "-" + date.toISOString().replace(/[.:-]/g, "").toLowerCase();
...
const SteemLine = new Vue({
    ...
    methods: {
        ...
        updateVotes: function () {
            if (this.account) {
                $.getJSON(sf.host + "api/votes?username=" + this.account.name + "&perpage=500", (response) => {
                    if (this.votes) {
                        const diff = this.votes.filter(votesDiff(response.votes));
                        if (diff.length) {
        ...
        submitStory: function (e) {
            ...
            const tags = this.currentDraftTags.toLowerCase().split(" ");
            const metaData = {tags: tags, app: "steemline/beta"};
